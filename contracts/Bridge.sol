...
contract Bridge is Pausable, AccessControl, SafeMath, MPTVerifier {
    ...
    /**
        ...
        @param dataHash Hash of data provided when deposit was made, along with proofs.
               Preimage is concatenation of: handler, data, rootHash, key, nodes,
               aggregatePublicKey, hashedMessage, signatureHeader.
    ...
    /**
        ...
        @param nodes is the RLP encoded list of MPT nodes (starting with the root) that need to be
    ...
    function executeProposal(
        ...
        ExecuteProposalMemory memory mem = ExecuteProposalMemory(
            chainID, depositNonce, resourceID, rootHash, address(0), 0, bytes32(0)
        );
        mem.handler = _resourceIDToHandlerAddress[mem.resourceID];
        mem.nonceAndID = (uint72(mem.depositNonce) << 8) | uint72(mem.chainID);
        mem.dataHash = keccak256(
            abi.encodePacked(mem.handler, data, mem.rootHash, key, nodes, aggregatePublicKey, hashedMessage, signatureHeader)
        );
