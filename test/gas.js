...
contract('Gas Benchmark - [Execute Proposal]', async (accounts) => {
    ...
    const execute = (depositNonce, depositData, resourceID) => BridgeInstance.executeProposal(chainID, depositNonce, depositData, resourceID, blockHeaderRLP, blockHashPrefix, blockHashSuffix, blockHashBLSHints, blockHashSignature, aggregatePublicKey, transactionMerkleKey, transactionMerkleNodes);
    ...
    it('Should execute proposal with 242 elements in the MPT', async () => {
        ...
        const fakeBlockHeaderRLP = '0xe4808080a0973a224acfb632a658cce34cc673ea07cd3897b9b050af01283ef16e7f657555';
        const fakeBlockHash = '0xc3a7929a44c79e524b7cfc25ea54907adcdbc0a6ffa38e8563ef7b638dd1f069';
        const depositNonce = 2;
        const depositData = Helpers.createERCDepositData(
            0,
            lenRecipientAddress,
            recipientAddress);
        const customPreimagePart = fakeBlockHash.slice(2) + localKey.slice(2) + aggregatePublicKey.slice(2);
        const depositDataHash = Ethers.utils.keccak256(ERC20HandlerInstance.address + depositData.substr(2) + customPreimagePart);

        await deposit(erc20ResourceID, depositData);
        await vote(erc20ResourceID, depositNonce, depositDataHash, relayerAddress);

        // Signature is not correct, so only works on BridgeGanache.
        const executeTx = await BridgeInstance.executeProposal(chainID, depositNonce, depositData, erc20ResourceID, fakeBlockHeaderRLP, blockHashPrefix, blockHashSuffix, blockHashBLSHints, blockHashSignature, aggregatePublicKey, localKey, localNodes);
    ...
    it('Should execute proposal with 50 elements in the MPT', async () => {
        ...
        const fakeBlockHeaderRLP = '0xe4808080a08649c3b1adbea6d8b4a8bc78ece9d78cea881e801bbea47bb8b5994401feeee1';
        const fakeBlockHash = '0x09ab48ab58eaf7502bce57b4f5b0d2204d8cff4c95fca13726b6a13685957c0a';
        const depositNonce = 2;
        const depositData = Helpers.createERCDepositData(
            0,
            lenRecipientAddress,
