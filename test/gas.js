...
// contract('Gas Benchmark - [Execute Proposal bytes[]]', async (accounts) => {
//     const chainID = 1;
//     const relayerThreshold = 1;
//     const relayerAddress = accounts[0];
//     const depositerAddress = accounts[1];
...
//     const rootHash = '0xff5c6287761305d7d8ae76ca96f6cb48e48aa04cf3c9280619c8993f21e335ca';
//     const key = signatureHeader;
//     const nodes = signatureHeader;

//     const preimage = rootHash.slice(2) + signatureHeader.slice(2).repeat(5);

//     const deposit = (resourceID, depositData) => BridgeInstance.deposit(chainID, resourceID, depositData, { from: depositerAddress });
//     const vote = (resourceID, depositNonce, depositDataHash) => BridgeInstance.voteProposal(chainID, depositNonce, resourceID, depositDataHash, { from: relayerAddress });
//     const execute = (depositNonce, depositData, resourceID) => BridgeInstance.executeProposalBytesArray(chainID, depositNonce, resourceID, rootHash, [depositData, signatureHeader, aggregatePublicKey, g1, hashedMessage, key, nodes]);
...
//             lenRecipientAddress,
//             recipientAddress);
//         const depositDataHash = Ethers.utils.keccak256(ERC20HandlerInstance.address + depositData.substr(2) + preimage);
...
//             lenMetaData,
//             metaData);
//         const depositDataHash = Ethers.utils.keccak256(ERC721HandlerInstance.address + depositData.substr(2) + preimage);
...
//         const hashOfCentrifugeAsset = Ethers.utils.keccak256('0xc0ffee');
//         const depositData = Helpers.createGenericDepositData(hashOfCentrifugeAsset);
//         const depositDataHash = Ethers.utils.keccak256(GenericHandlerInstance.address + depositData.substr(2) + preimage);
...
// contract('Gas Benchmark - [Execute Proposal RLP]', async (accounts) => {
//     const chainID = 1;
//     const relayerThreshold = 1;
//     const relayerAddress = accounts[0];
//     const depositerAddress = accounts[1];
...
//     const deposit = (resourceID, depositData) => BridgeInstance.deposit(chainID, resourceID, depositData, { from: depositerAddress });
//     const vote = (resourceID, depositNonce, depositDataHash) => BridgeInstance.voteProposal(chainID, depositNonce, resourceID, depositDataHash, { from: relayerAddress });
//     const execute = (depositNonce, depositData, resourceID) => BridgeInstance.executeProposalRLP(chainID, depositNonce, resourceID, rootHash, hexlify(rlp.encode([arrayify(depositData), arrayify(signatureHeader), arrayify(aggregatePublicKey), arrayify(g1), arrayify(hashedMessage), arrayify(key), arrayify(nodes)])));
